...
import type { TrackingData } from "./tracker";
...
export function filterJobs(pool: Job[], q: MultiFilterQuery, now: Date = new Date()): Job[] {
  ...
  if (!jobTypes.includes("all")) {
    // Check if "收藏" is selected
    const hasSavedFilter = (jobTypes as string[]).includes("收藏");
    const otherTypes = (jobTypes as string[]).filter(t => t !== "收藏");
    
    if (hasSavedFilter) {
      // Filter jobs that are saved in tracking
      out = out.filter((j) => tracking[j.id]?.status === "saved");
    } else if (otherTypes.length > 0) {
      out = out.filter((j) => otherTypes.includes(j.jobType));
    }
  }
