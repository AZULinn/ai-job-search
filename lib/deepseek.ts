export type AiProvider = "deepseek" | "openai" | "qwen" | "zhipu" | "moonshot";
...
export const AI_PROVIDERS: AiProviderConfig[] = [
  {
    ...
    name: "DeepSeek",
    models: [
      { id: "deepseek-chat", name: "DeepSeek-V4 (deepseek-chat)" },
  ...
  },
  {
    ...
    name: "OpenAI",
    models: [
      { id: "gpt-5.6", name: "GPT-5.6 Sol (旗舰)" },
      { id: "gpt-5.6-terra", name: "GPT-5.6 Terra (均衡)" },
      { id: "gpt-5.6-luna", name: "GPT-5.6 Luna (高效)" },
  ...
  },
  {
    ...
    name: "通义千问",
    models: [
      { id: "qwen3.8-max", name: "Qwen3.8-Max (旗舰)" },
      { id: "qwen3.7-plus", name: "Qwen3.7-Plus (均衡)" },
      { id: "qwen3.7-flash", name: "Qwen3.7-Flash (极速)" },
  ...
  },
  {
    ...
    name: "智谱 AI",
    models: [
      { id: "glm-5.3", name: "GLM-5.3 (旗舰)" },
      { id: "glm-5.3-flash", name: "GLM-5.3-Flash (极速)" },
  ...
  },
  {
    ...
    name: "Moonshot (Kimi)",
    models: [
      { id: "kimi-k3", name: "Kimi K3 (1M context)" },
...
const SETTINGS_KEY = "career-search:ai-settings:v1";
...
export function getSettings(): AiSettings {
  if (typeof window === "undefined") return { provider: "deepseek", apiKey: "" };
  try {
...
export async function callDeepSeek<T>(
  ...
  try {
    ...
    if ((e as Error).name === "AbortError") {
      throw new Error("AI 生成超时，请检查网络后重试。");
    }
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({})) as { error?: string; raw?: string };
    const msg = err.error ?? `AI 调用失败 (${res.status})`;
...
// ─── Prompt: 简历解析 ───
export const RESUME_PROMPT = `你是简历解析专家。从简历中提取结构化信息，特别注意完整提取每段实习/工作经历的细节。
返回 JSON：
{
  "school": "学校名",
  "major": "专业",
...
// ─── Prompt: 面试题定制 ───
export const INTERVIEW_PROMPT = `你是一个资深面试官。根据候选人背景和目标岗位，生成针对性面试题。
返回 JSON：
...
// ─── Prompt: 面试题追问 ───
export const INTERVIEW_FOLLOWUP_PROMPT = `你是一个资深面试官。根据候选人背景、已有面试题和用户追问，生成新的针对性面试题。
不要重复已有题目。返回 JSON：
{"questions":[{"question":"面试题","category":"技术|业务|行为|情景","difficulty":"简单|中等|困难","tips":"回答要点提示（50-100字）","sample":"参考答案要点（100-200字，结构化回答）"}]}
生成 3-5 道新题，紧扣用户追问方向。sample 必须是详细的参考答案。`;

// ─── Prompt: 简历润色 ───
export const RESUME_POLISH_PROMPT = `你是简历优化专家。根据候选人的背景和目标岗位，对每段经历给出具体的润色建议。
返回 JSON：
...
// ─── Prompt: 简历优化（合并版+方向建议）───
export const RESUME_OPTIMIZE_PROMPT = `你是简历优化专家。根据候选人背景、目标岗位 JD 和原始简历/经历，完成三件事：
...
// ─── Prompt: 求职信生成 ───
export const COVER_LETTER_PROMPT = `你是求职信写作专家。根据候选人画像和目标岗位，生成一封专业的中文求职信。
返回 JSON：
{"letter":"完整的求职信正文（400-600字）","highlights":["信中突出的3个核心卖点"],"tips":"投递注意事项"}
求职信要自然真诚，不要套话。开头要有亮点抓注意力。中间匹配岗位要求。结尾表达诚意。`;

// ─── Prompt: 求职信修改 ───
export const COVER_LETTER_REFINE_PROMPT = `你是求职信写作专家。你将收到一封已有的求职信、候选人画像、目标岗位信息，以及用户的修改指令。
...
// ─── Prompt: 简历修改 ───
export const RESUME_REFINE_PROMPT = `你是简历优化专家。你将收到一份已有的简历内容（分段格式）、候选人画像、目标岗位信息，以及用户的修改指令。
...
// ─── Prompt: Offer 对比 ───
export const OFFER_COMPARE_PROMPT = `你是职业规划专家。帮候选人对比分析多个 Offer，给出最终建议。
返回 JSON：
...
// ─── Prompt: JD 匹配分析 ───
export const JD_MATCH_PROMPT = `你是简历与岗位匹配分析专家。根据候选人画像和目标岗位 JD，进行深度匹配分析。
返回 JSON：
{
  "matchScore": 78,
  "modules": [
...
// ─── Prompt: 一键定制简历 ───
export const CUSTOM_RESUME_PROMPT = `你是简历定制专家。根据候选人背景和目标岗位 JD，生成一份针对该岗位定制的简历。
返回 JSON：
{
  "sections": [
...
// ─── Prompt: 多 JD 对比排序 ───
export const JD_COMPARE_PROMPT = `你是职业规划专家。根据候选人画像和多个目标岗位 JD，分析匹配度并给出投递优先级排序。
返回 JSON：
{
  "rankings": [
...
// ─── Prompt: 方向模版参考 ───
export const DIRECTION_TEMPLATE_PROMPT = `你是资深职业顾问。根据候选人背景和目标求职方向，生成一份该方向的简历模版和策略建议。
返回 JSON：
{
...
// ─── Prompt: 投递进展 AI 分析 ───
export const PROGRESS_ANALYZE_PROMPT = `你是求职数据分析专家。根据候选人的求职进展数据，给出深度分析和建议。
返回 JSON：
{
...
// ─── Prompt: AI 求职教练 ───
export const COACH_PROMPT = `你是一名资深求职策略教练。根据候选人的完整求职数据（画像、投递记录、面试记录、新岗位），给出精准的今日行动建议。
...
// ─── Prompt: 面试记录 AI 解析 ───
export const INTERVIEW_PARSE_PROMPT = `你是一个面试记录助手。从用户的自然语言描述中提取结构化面试信息。
返回 JSON：
{
  "company": "公司名（母公司/集团名）",
  "position": "岗位名",
